package valeriafarinosi.U5_W2_D3.controllers;


import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.*;
import valeriafarinosi.U5_W2_D3.entities.BlogAuthor;
import valeriafarinosi.U5_W2_D3.payloads.AuthorRequestPayload;
import valeriafarinosi.U5_W2_D3.payloads.AuthorResponsePayload;
import valeriafarinosi.U5_W2_D3.services.AuthorService;
...
public class AuthorController {
    ...
    public AuthorResponsePayload createAuthor(@RequestBody AuthorRequestPayload payload) {
        BlogAuthor saved = this.authorService.saveAuthor(payload);
        return new AuthorResponsePayload(saved.getAuthorId());
    }

    // 4. PUT http://localhost:3003/authors/{authorId} + payload -> update the BlogAuthor with id = authorId
    @PutMapping("/{authorId}")
    public BlogAuthor findByIdAndUpdate(@PathVariable int authorId, @RequestBody AuthorRequestPayload payload) {
