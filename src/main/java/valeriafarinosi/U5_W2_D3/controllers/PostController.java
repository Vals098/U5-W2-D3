...
import valeriafarinosi.U5_W2_D3.payloads.PostResponsePayload;
import valeriafarinosi.U5_W2_D3.services.PostService;
...
public class PostController {
    ...
    public PostResponsePayload createPost(@RequestBody PostRequestPayload payload) {
        BlogPost saved = this.postService.savePost(payload);
        return new PostResponsePayload(saved.getPostId());
    }

    // 4. PUT http://localhost:3003/blogPosts/{postId} + payload -> update the BlogPost with id = postId
    @PutMapping("/{postId}")
    public BlogPost findByIdAndUpdate(@PathVariable int postId, @RequestBody PostRequestPayload payload) {
